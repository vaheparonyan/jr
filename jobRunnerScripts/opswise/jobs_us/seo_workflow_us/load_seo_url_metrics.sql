DELETE FROM sandbox.as_seo_url_metrics WHERE event_day_key = ${date_key};
...
WHERE m.month_key = ( SELECT month_key FROM user_groupondw.dim_day WHERE day_key = ${date_key} );
...
WHERE dd.month_key = ( SELECT month_key FROM user_groupondw.dim_day WHERE day_key = ${date_key} )
