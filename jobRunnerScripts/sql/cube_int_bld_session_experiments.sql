DELETE FROM seo_analytics.cube_int_bld_session_experiments where date_value = '@parameter_date';

SQL_BREAK;

REPLACE INTO seo_analytics.cube_int_bld_session_experiments
...
select
    ...
    ,COALESCE(sm.experiment,'none')
    ,COALESCE(sm.variant, 'none')
    ,COALESCE(sm.layer, 'none')
...
from
    seo_analytics.int_bld_session_experiments_fcs sm
