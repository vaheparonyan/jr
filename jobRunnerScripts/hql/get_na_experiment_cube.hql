...
INSERT OVERWRITE TABLE  ${db_name}.c_na_experiment_data_cube PARTITION (dt='${date_value}')
...
select
    ...
    ,COALESCE(ex.experiment,'none')
    ,COALESCE(ex.variant, 'none')
    ,COALESCE(ex.layer, 'none')
...
from
    ...
    left outer join seo_cubes.c_session_orders od on (od.session_id = fp.session_id and od.dt = fp.dt)
    left outer join seo_cubes.c_subscribers_subscriptions ss on (ss.session_id = fp.session_id and ss.dt = fp.dt) 
    left outer join seo_cubes.c_page_and_traffic_classification tc on (tc.session_id = fp.session_id and tc.dt = fp.dt)
    left outer join seo_cubes.c_mktg_ref_key_attributes rf on (rf.session_id = fp.session_id and rf.dt = fp.dt)
    left outer join seo_cubes.c_experiments_data ex on (ex.session_id = fp.session_id and ex.user_browser_id = fp.user_browser_id and ex.dt = fp.dt)
where
    fp.dt = '${date_value}' 
group by
    dd.is_holiday
